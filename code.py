...
class Filter(object):
    ...
    def __init__(self, array_size, hashes):
...
if __name__ == "__main__":
    print "Bloom filter Data Structure"
    array_size_i = input("Enter your bloom filter array size in byte: ")
    hashes_i = input("Enter number of hashes you want to use : ")
    bf = Filter(array_size_i, hashes_i)

    action = None
    print "Add new value to bloom filter like this: add 1234"
    print "Check whether value is present in set using check 1234"
    print "End program by typing anything"

    while True:
        input_text = raw_input().split()
        if input_text[0] == "add":
            bf.add(input_text[1])
        elif input_text[0] == "check":
            print "Probably yes" if bf.query(input_text[1]) else "No"
        else:
            break
